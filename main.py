...
class GroupFileCheckerPlugin(Star):
    def __init__(self, context: Context, config: Optional[Dict] = None):
        ...
        self._temp_dir = os.path.join(get_astrbot_data_path(), "plugins_data", "file_checker", "temp")
    ...
    async def _repack_and_send_txt(self, event: AstrMessageEvent, original_filename: str, local_file_path: str):
        ...
        try:
            ...
            repacked_file_path = os.path.join(self._temp_dir, f"{int(time.time())}_{new_zip_name}")
    ...
    async def _get_local_file_from_event(self, event: AstrMessageEvent, file_component: Comp.File) -> Optional[str]:
        group_id = int(event.get_group_id())
        ...
        try:
            ...
            logger.error(f"[{group_id}] 下载文件 '{file_name}' 时发生错误: {e}", exc_info=True)
            return None
    ...
    async def _handle_file_check_flow(self, event: AstrMessageEvent, file_name: str, file_id: str, file_component: Comp.File):
        ...
        try:
            ...
            if is_gfs_valid:
                ...
                asyncio.create_task(self._task_delayed_recheck(event, file_name, file_id, local_file_path, preview_text))
            else:
            ...
            logger.error(f"[{group_id}] 处理文件时发生错误: {e}", exc_info=True)
    ...
    async def _get_preview_for_file(self, file_name: str, local_file_path: str) -> tuple[str, str]:
        is_txt = file_name.lower().endswith('.txt')
        ...
        try:
            ...
            logger.error(f"获取预览时读取文件失败: {e}", exc_info=True)
            return "", ""
        
        return "", ""

    async def _task_delayed_recheck(self, event: AstrMessageEvent, file_name: str, file_id: str, local_file_path: str, preview_text: str):
    ...
    async def terminate(self):
        ...
        if os.path.exists(self._temp_dir):
            try:
                ...
                logger.info(f"已清理临时文件夹: {self._temp_dir}")
            except OSError as e:
